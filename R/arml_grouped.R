...
#' @param horizons A numeric vector of one or more forecast horizons, h,
#' measured in dataset rows.If dates are given, a horizon of 1, for example,
#' would equal 1 * frequency in calendar time.
#' @param lookback A numeric vector giving the lags–in dataset rows–for creating
#' the lagged features.
...
#' @param train_control Control parameters for caret train see
#' \code{\link[caret]{trainControl}}
#' @param ncore If \code{allowParallel = TRUE} in
#' \code{\link[caret]{trainControl}}
...
#' @param return_fitted_values Boolean. If \code{TRUE}fitted values will be
#' returned.
...
ARml_grouped <- function(data, outcome_col_name,
                         ...
                         frequency = "1 month",
                         groups = NULL,
                         dynamic_features = c("month", "year", "week", "day",
                                              "quarter"),
                         ...
                         set_seed = 42)
{
  ...
  data_forecast <- forecastML::create_lagged_df(
    data, type = "forecast",
    method = "direct",
    outcome_col = outcome_col,
    horizons = horizons,
...
data_forecast[[i]]$quarter <- (lubridate::quarter(data_forecast[[i]]$index))
  ...
  forecast_all_horizones <- predict(
    model_results,
    prediction_function = list(prediction_function),  data = data_forecast)
